...
import { marqueeArtists, heroContent } from "@/data/distrokid-data";

// Build marquee text from real artist names
const topTicker = marqueeArtists.map((a) => a.toUpperCase()).join(" // ") + " // ";
const bottomTicker = `OWN YOUR SOUND // KEEP 100% // ${heroContent.stats.map((s) => `${s.value} ${s.label.toUpperCase()}`).join(" // ")} // INDEPENDENCE NOW // `;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-center max-w-4xl px-6 flex flex-col items-center">
        ...
        >
          Join {heroContent.stats[0].value} Artists
        </motion.span>
        ...
        <Magnetic range={80} pullStrength={0.3}>
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center">
              <span className="font-mono text-[9px] md:text-[10px] font-black uppercase tracking-wider block mb-1">{heroContent.ctaPrimary.split(" ")[0].toUpperCase()}</span>
              <span className="font-sans text-xs md:text-sm font-extrabold uppercase tracking-tight block">{heroContent.ctaPrimary.split(" ").slice(1).join(" ").toUpperCase() || "NOW"}</span>
